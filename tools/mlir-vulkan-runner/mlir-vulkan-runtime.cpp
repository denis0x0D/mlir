//===- mlir-vulkan-runtime.cpp - MLIR Vulkan Execution Driver---------------===//
//
...
static LogicalResult countMemorySize(
...
static LogicalResult fetchBuffersData(
    const VkDevice &device,
    llvm::ArrayRef<VulkanDeviceMemoryBuffer> memoryBuffers,
    llvm::DenseMap<Descriptor, VulkanBufferContent> &bufferContents) {
  for (auto memoryBuffer : memoryBuffers) {
    if (bufferContents.count(memoryBuffer.descriptor)) {
      float *payload;
      auto bufferContent = bufferContents[memoryBuffer.descriptor];
      RETURN_ON_VULKAN_ERROR(vkMapMemory(device, memoryBuffer.deviceMemory, 0,
                                         bufferContent.size, 0,
                                         (void **)&payload),
                             "vkMapMemory");
      std::memcpy(bufferContent.ptr, payload, bufferContent.size);
      vkUnmapMemory(device, memoryBuffer.deviceMemory);
    } else {
      // TODO: Is it possible to face this situation?
      return failure();
    }
  }
  return success();
