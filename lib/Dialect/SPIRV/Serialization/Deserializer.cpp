  ...
  case spirv::Decoration::Block:
    if (words.size() != 2) {
      return emitError(unknownLoc, "OpDecoration with ")
             << decorationName << "must have 2 operands";
    }
...
LogicalResult Deserializer::processMemberDecoration(ArrayRef<uint32_t> words) {
  ...
  case spirv::Decoration::Offset:
    ...
    break;
  default:
    return emitError(unknownLoc, "unhandled OpMemberDecoration with op code: '")
           << words[2];
  }
  return success();
}
...
LogicalResult Deserializer::processStructType(ArrayRef<uint32_t> operands) {
  if (operands.size() < 2) {
    return emitError(unknownLoc, "OpTypeStruct must have at least one member");
  }

  SmallVector<Type, 0> memberTypes;
  for (uint32_t opNum = 1; opNum < operands.size(); ++opNum) {
    Type memberType = getType(operands[opNum]);
    if (!memberType) {
  ...
  auto memberDecorationIterator = memberDecorationMap.find(operands[0]);
  if (memberDecorationIterator != memberDecorationMap.end()) {
    // Each member must have an offset
    SmallVector<spirv::StructType::LayoutInfo, 0> layoutInfo;
    const auto &offsetDecorationMap = memberDecorationIterator->second;
    for (uint32_t memberNum = 0; memberNum < memberTypes.size(); ++memberNum) {
      // Check that specific member has an offset
      auto offsetIterator = offsetDecorationMap.find(memberNum);
      if (offsetIterator != offsetDecorationMap.end()) {
        layoutInfo.push_back(
            static_cast<spirv::StructType::LayoutInfo>(offsetIterator->second));
      } else {
        return emitError(unknownLoc, "OpTypeStruct with <id> ")
               << operands[0] << " must have an offset for member with number "
               << memberNum;
      }
    }
    typeMap[operands[0]] = spirv::StructType::get(memberTypes, layoutInfo);
  } else {
    typeMap[operands[0]] = spirv::StructType::get(memberTypes);
  }
  return success();
}
